...
async def create_post_for_user(controller: Annotated[PostController, Depends()], post: schemas.PostCreate) -> models.Post:
    return await controller.create(post=post)


@router.get("/posts/", response_model=list[schemas.Post])
async def read_posts(repo: PostRepository, skip: int = 0, limit: int = 100) -> list[models.Post]:
